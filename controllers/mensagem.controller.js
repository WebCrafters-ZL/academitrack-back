...
const enviarMensagemParaTodasTurmas = async (req, res) => {
  try {
    ...
    res.status(201).json({ mensagem: 'Mensagem enviada com sucesso para todas as turmas', mensagem: novaMensagem });
  } catch (erro) {
    console.error(erro);
    res.status(500).json({ mensagem: 'Erro ao enviar mensagem para todas as turmas', erro: erro.message });
  }
};
...
const listarMensagensParaTodasTurmas = async (req, res) => {
  try {
    ...
    res.status(500).json({ mensagem: 'Erro ao listar mensagens', erro: erro.message });
  }
};
...
const excluirMensagemDeTodasTurmas = async (req, res) => {
  try {
    ...
    res.status(200).json({ mensagem: 'Mensagem deletada com sucesso de todas as turmas' });
  } catch (erro) {
    console.error(erro);
    res.status(500).json({ mensagem: 'Erro ao deletar mensagem de todas as turmas', erro });
  }
};
...
const enviarMensagemParaTurma = async (req, res) => {
  try {
    ...
    await novaMensagem.save();
    res.status(201).json({ mensagem: 'Mensagem enviada com sucesso para a turma específica', mensagem: novaMensagem });
  } catch (erro) {
    console.error(erro);
    res.status(500).json({ mensagem: 'Erro ao enviar mensagem para a turma', erro: erro.message });
  }
};
...
const listarMensagensPorTurma = async (req, res) => {
  try {
    ...
    console.error('Erro ao listar mensagens:', erro);
    res.status(500).json({ mensagem: 'Erro ao listar mensagens para a turma', erro: erro.message });
  }
};
...
const excluirMensagemDaTurma = async (req, res) => {
  try {
    ...
    if (!mensagem.destinatario_id.includes(turmaId)) {
      return res.status(400).json({ mensagem: 'Mensagem não pertence a esta turma' });
    }

    await Mensagem.findByIdAndDelete(mensagemId);
    res.status(200).json({ mensagem: 'Mensagem deletada com sucesso da turma específica' });
  } catch (erro) {
    console.error(erro);
    res.status(500).json({ mensagem: 'Erro ao deletar mensagem da turma', erro });
  }
};
